...
void pfls_MassErase( uint32 start_address )
{
	...
	end_address = start_address + 0x400000;
    for (uint32 pfls_sector_start = start_address; pfls_sector_start < end_address; pfls_sector_start += PFLASH_LOGICAL_SECTOR_SIZE)
    {
...
void swap_update(void)
{
    ...
    int entry_num = ((scu_stmem1 & 0x00F80000) >> 19) / 2;
    uint32 addr_cfg = (swapctrl & 0x00000003);
